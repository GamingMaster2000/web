...
module.exports = (TrackChangesController = {
	...
	acceptChange(req, res, next) {
		const {project_id, doc_id, change_id} = req.params;
		logger.log({project_id, doc_id, change_id}, "request to accept change");
		return DocumentUpdaterHandler.acceptChange(project_id, doc_id, change_id, function(error) {
			if (error != null) { return next(error); }
			EditorRealTimeController.emitToRoom(project_id, "accept-change", doc_id, change_id, function(err){});
			return res.send(204);
		});
	},
